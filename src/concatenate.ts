...
export const concatFiles = async ({
  ...
  const filteredFiles = files
    .filter(
      (file) =>
        !ignoreFilesRegExps.some((regexp: RegExp) => {
    ...
    .filter(
      (file) =>
        !/\.(png|jpe?g|gif|svg|webp|ico|bmp|tiff|psd|raw|heif|indd|ai|eps|pdf|xcf|sketch|fig|xd)$/i.test(
          file,
        ),
    );

  // 拡張子が一致するファイルのみを抽出
  const targetFiles = type
    ? filteredFiles.filter((file) => file.endsWith(type))
    : filteredFiles;
...
`;
  ...
  // 出力ファイル名を生成
  const fileName = `${pascalCase(dir).replace(/[^a-zA-Z0-9]/g, "")}${
    type ? `.${type}` : ""
  }.${Date.now()}.md`;
